...
UniValue getpeerinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "    \"inflight\": [\n"
            "       n,                        (numeric) The heigBTS of blocks we're currently asking from this peer\n"
            "       ...\n"
            "    ]\n"
            "    \"bytessent_per_msg\": {\n"
    ...
    BOOST_FOREACH(const CNodeStats& stats, vstats) {
        ...
        if (fStateStats) {
            ...
            UniValue heigBTS(UniValue::VARR);
            BOOST_FOREACH(int height, statestats.vHeightInFlight) {
                heigBTS.push_back(height);
            }
            obj.push_back(Pair("inflight", heigBTS));
        }
...
UniValue getaddednodeinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:5556\",  (string) The BTS server IP and port we're connected to\n"
...
UniValue addanonserver(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 3 ||
        ...
        throw runtime_error(
            ...
            "\nAttempts add or remove a node from the BTSTech server list.\n"
            "\nArguments:\n"
            "1. \"node\"     (string, required) The node (see getpeerinfo for nodes)\n"
    ...
    if (strCommand == "add")
    {
      if (params.size() == 3 && params[2].get_str() == "true") {
        ...
        if (it != vAddedAnonServers.end())
          throw JSONRPCError(RPC_CLIENT_NODE_ALREADY_ADDED, "Error: BTSTech server already added");
        vAddedAnonServers.push_back(strNode);
      }
    }
    else if(strCommand == "remove")
    {
      if (params.size() == 3 && params[2].get_str() == "true") {
        ...
        if (it == vAddedAnonServers.end())
          throw JSONRPCError(RPC_CLIENT_NODE_NOT_ADDED, "Error: BTSTech server has not been added.");
        vAddedAnonServers.erase(it);
      }
    }
    return NullUniValue;
...
UniValue getstakesubsidy(const UniValue& params, bool fHelp)
{
    ...
    try {
        ...
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "BTS decode failed");
    }
