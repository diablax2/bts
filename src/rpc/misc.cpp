...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total BTS balance of the wallet\n"
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                a.push_back(CBTSAddress(addr).ToString());
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"BTSaddress\"\n"
            "\nReturn information about the given BTS address.\n"
            "\nArguments:\n"
            "1. \"BTSaddress\"     (string, required) The BTS address to validate. This may also be an OpenAlias address.\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"BTSaddress\", (string) The BTS address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: BTS address and we have full public key:
        CBTSAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are BTS addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) BTS address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue createwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    CBTSAddress address(witscriptid);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"BTSaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"BTSaddress\"  (string, required) The BTS address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CBTSAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CBTSAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CBTSAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
bool heigBTSort(std::pair<CAddressUnspentKey, CAddressUnspentValue> a,
...
UniValue getaddressutxos(const UniValue& params, bool fHelp)
{
    ...
    std::sort(unspentOutputs.begin(), unspentOutputs.end(), heigBTSort);
