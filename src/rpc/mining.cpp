...
UniValue generateBlocks(boost::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    {   // Don't keep cs_main locked
        LOCK(cs_main);
        nHeigBTStart = chainActive.Height();
        nHeight = nHeigBTStart;
        nHeightEnd = nHeigBTStart+nGenerate;
    }
...
UniValue generatetoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. numblocks    (numeric, required) How many blocks are generated immediately.\n"
            "2. address    (string, required) The address to send the newly generated BTS to.\n"
...
// NOTE: Unlike wallet RPC (which use BTS values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    https://github.com/BTS/bips/blob/master/bip-0022.mediawiki\n"
            "    https://github.com/BTS/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "BTS is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "BTS is downloading blocks...");
...
UniValue submitblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "See https://en.BTS.it/wiki/BIP_0022 for full specification.\n"
...
UniValue estimatesmartfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric) estimate fee-per-kilobyte (in BTS)\n"
