...
const std::string BTSGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString BTSGUI::DEFAULT_WALLET = "~Default";

BTSGUI::BTSGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
void BTSGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/BTS"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a BTS address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and BTS: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BTS addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BTS addresses"));
    ...
    openAction->setStatusTip(tr("Open a BTS: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BTS command-line options").arg(tr(PACKAGE_NAME)));
...
void BTSGUI::bootstrapBlockchain()
{
    bool ok = false;
    QString defaultUrl = "https://nav.nyc3.digitaloceanspaces.com/bootstrap/mainnet/bootstrap-BTS_" +
            QString::fromStdString(Params().NetworkIDString()) + "net.tar";
    QString url = QInputDialog::getText(this, tr("Bootstrap blockchain"),
                                            tr("You can use an external trusted source to download the blockchain from.<BR>The following URL points to a bootstrap copy provided by the BTS Core Team.<BR>Where would you like to download it from?"), QLineEdit::Normal,
                                            defaultUrl, &ok);
    if (ok && !url.isEmpty())
    {
...
void BTSGUI::createMenuBar()
{
    ...
    if(walletFrame)
    {
        ...
        {
            QAction *menuAction = new QAction(QString(BTSUnits::name(u)), this);
...
void BTSGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void BTSGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool BTSGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void BTSGUI::startVotingCounter()
{
    if (GetStaking())
    {
...
bool BTSGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void BTSGUI::setWalletActionsEnabled(bool enabled)
{
...
void BTSGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void BTSGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void BTSGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BTSGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void BTSGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void BTSGUI::openClicked()
{
...
void BTSGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void BTSGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void BTSGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void BTSGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BTSGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void BTSGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to BTS network", "", count));
    if(walletFrame){
...
void BTSGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to BTS network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to BTS network..."));
...
void BTSGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("BTS"); // default title
...
void BTSGUI::changeEvent(QEvent *e)
{
...
void BTSGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BTSGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BTSGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BTSUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void BTSGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BTSGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BTSGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BTSGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BTSGUI::setEncryptionStatus(int status)
{
...
void BTSGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BTSGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BTSGUI::detectShutdown()
{
...
void BTSGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void BTSGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(BTSGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void BTSGUI::subscribeToCoreSignals()
{
...
void BTSGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<BTSUnits::Unit> units = BTSUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const BTSUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(BTSUnits::name(unit)));
    }
...
{
    setText(BTSUnits::name(newUnits));
}
...
void BTSGUI::replyFinished(QNetworkReply *reply)
{
...
void BTSGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CBTSAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void BTSGUI::getVotingInfo()
{
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.BTS.org/voting.") + QString::fromStdString(Params().NetworkIDString()) + QString("net.json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
...
void BTSGUI::updateStakingStatus()
{
    updateWeight();
