...
const char * const BTS_CONF_FILENAME = "BTS.conf";
const char * const BTS_PID_FILENAME = "BTS.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BTS
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BTS
    // Mac: ~/Library/Application Support/BTS
    // Unix: ~/.BTS
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BTS4";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BTS4";
#else
    // Unix
    return pathRet / ".BTS4";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", BTS_CONF_FILENAME));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override BTS.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", BTS_PID_FILENAME));
