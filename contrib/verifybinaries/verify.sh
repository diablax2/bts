...
###   This script attempts to download the signature file SHA256SUMS.asc from BTS.org
...
RCSUBDIR="test/"
BASEDIR="https://BTS.org/bin/"
VERSIONPREFIX="BTS-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'BTS-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BTS Core binary release signing key installed?"
   fi
