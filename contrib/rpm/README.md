...
The RPM spec file provided here is for BTS-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://BTS.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[BTS git repository](https://github.com/BTS/BTS) but are not part of
...
Currently only `Source10` is used, for the example `BTS.conf` file.
...
in the BTS git repository. At present only an SVG version of the BTS
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the BTS
developers so that an official fix to the issue can make it into the next
release.

### Patch0 BTS-0.12.0-libressl.patch
...
needed when 0.12.1 is released, a proper fix is already in the BTS git
master branch.
...
idea to build BTS Core against a static version of that library compiled
...
The `BTS.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `BTS-server` package. At this point, most RPM based
...
The `BTS-server` package should have SELinux support. How to properly do
...
BTS-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/BTS/). He has been
packaging BTS for Fedora at least since 2012.
...
4. I renamed the `BTS` package that contains the Qt GUI to `BTS-core` as
that appears to be how the general population refers to it, in contrast to
`BTS-xt` or `BTS-classic`. I wanted to make sure the general population
