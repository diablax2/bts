...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete BTS database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no BTS datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old BTS datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected BTS 0.7 datadir."
    ;;
  3)
    echo "Detected BTS pre-0.8 datadir."
    ;;
  4)
    echo "Detected BTS 0.8 datadir."
    ;;
esac
